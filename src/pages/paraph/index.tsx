...
const paraphCtx = React?.createContext({});
...
const ParaphView = ({}) => {
  ...
  React?.useEffect(() => {
    ...
    (async () => {
      ...
      const filePath = JSON.parse(sessionStorage?.getItem("paraph-ctx"))?.data
        ?.path;
        ...
        .then((data) =>
          data
            ...
            .then((file) => {
              ...
              const name =
                JSON.parse(sessionStorage?.getItem("paraph-ctx"))?.data?.name ||
                `Paraphe`;
  ...
  const fileElement = {
    0: (
      <Stack id={"File"} component={"main"} key={window?.location?.pathname}>
        <EditFile
          // key={params?.id}
          injectDate={true}
          pdfName={pdfName}
  ...
  return (
    ...
    >
      <Layout>
        {/* we add the key prop for rerendering when same level path params change*/}
